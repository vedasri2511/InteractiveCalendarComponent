...
function buildRangeLabel(start: Date | null, end: Date | null) {
  if (!start && !end) return "Pick a start date and an end date.";
  if (start && !end) return `${format(start, "EEE, MMM d")} • choose an end date`;
...
export default function Calendar() {
  ...
  const turnTimersRef = useRef<{ enter: number | null; exit: number | null }>({ enter: null, exit: null });
  ...
  const selectedCount = useMemo(() => {
    if (!selectedRange.start || !selectedRange.end) return selectedRange.start ? 1 : 0;
  ...
  useEffect(() => {
    return () => {
      if (turnTimersRef.current.enter) window.clearTimeout(turnTimersRef.current.enter);
      if (turnTimersRef.current.exit) window.clearTimeout(turnTimersRef.current.exit);
    };
  }, []);
  ...
  const cardClasses = [
    ...
    isTurning && turnDirection === "next" ? "is-next" : "",
    isTurning && turnDirection === "prev" ? "is-prev" : "",
  ]
    .filter(Boolean)
    .join(" ");
        ...
        <div className="calendar-layout">
          <div className="calendar-content">
            <div className="calendar-toolbar">
              <div>
                <span className="calendar-toolbar-label" style={{ fontSize: "0.6rem", textTransform: "uppercase", letterSpacing: "0.14em", color: "rgba(255,255,255,0.7)", fontWeight: 700 }}>
                  Current sheet
                </span>
                <h2>{format(viewMonth, "MMMM yyyy")}</h2>
              </div>
              <div className="calendar-actions">
                <button onClick={() => jumpMonth(-1)} className="calendar-nav-button">
                  ←
                </button>
                <button onClick={resetSelection} className="calendar-secondary-button">
                  Clear range
                </button>
                <button onClick={() => jumpMonth(1)} className="calendar-nav-button">
                  →
            ...
            <div className="calendar-grid">
              {days.map((day) => (
                <Day
                  ...
                  isSelectedStart={!!selectedRange.start && isSameDay(day, selectedRange.start)}
                  isSelectedEnd={!!selectedRange.end && isSameDay(day, selectedRange.end)}
